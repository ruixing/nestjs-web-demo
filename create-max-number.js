...
var maxNumber = function(nums1, nums2, k) {
  const cache = { max: [] };
  findLeadNumber(nums1, nums2, k, [], cache);
  console.log(cache.max);
  return cache.max;
};

const findLeadNumber = (nums1, nums2, k, res = [], cache = { max: [] }) => {
  if (k === 0) {
    if (cache.max.length > 0) {
      cache.max = biggerThan(cache.max, res) ? cache.max : res;
    } else {
      cache.max = res;
    }
    return;
  }
  if (cache.max.length > 0 && biggerThan(cache.max.slice(0, res.length), res)) {
    return;
  }
  for (let i = 9; i >= 0; i--) {
    const nums1Index = checkNumberInScope(nums1, i, k - nums2.length);
    const nums2Index = checkNumberInScope(nums2, i, k - nums1.length);
    if (nums1Index !== -1 && nums2Index === -1) {
      res.push(i);
      findLeadNumber(nums1.slice(nums1Index + 1), nums2, k - 1, res, cache);
      return;
    } else if(nums2Index !== -1 && nums1Index === -1) {
      res.push(i);
      findLeadNumber(nums1, nums2.slice(nums2Index + 1), k - 1, res, cache);
      return;
    } else if (nums1Index !== -1 && nums2Index !== -1) {
      findLeadNumber(nums1, nums2.slice(nums2Index + 1), k - 1, [...res, i], cache);
      findLeadNumber(nums1.slice(nums1Index + 1), nums2, k - 1, [...res, i], cache);
      return;
    }
  }
}

const checkNumberInScope = (nums, target, k) => {
  for (let i = 0; i < nums.length; i++) {
    if (nums[i] === target && nums.length - i >= k) {
      return i;
    }
  }
  return -1;
...
maxNumber([8,9], [3,9], 3)
maxNumber([3,9], [8,9], 3)
// maxNumber([3,3,9,7,5], [8,9,7,5,8,3], 7)
maxNumber(
